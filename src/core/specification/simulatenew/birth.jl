...
function _givesbirth!(woman,model)
    ...
    baby = Person(nextid(model),pos=home(woman),
...
function _birth!(woman, model, currstep, data, birthpars, popfeature,caching)
...
function birth!(woman, model, popfeature::PopulationFeature = FullPopulation();
    ...
    if _birth!(woman, model, currenttime(model), data_of(model), birth_pars(model), popfeature, caching)
...
function _dobirths!(ret, model, popfeature, caching::CachingOperation )
    ...
    for (ind,woman) in enumerate(Iterators.reverse(people))
        if _birth!(woman, model, currenttime(model), data, birthpars, popfeature, caching)
